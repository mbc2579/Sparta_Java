...
public class Main {
    public static void main(String[] args) {
        ...
        // 카테고리가 여행인 책의 제목
        bookList.stream().filter(category ->category.getCategory().equals("여행")).forEach(cate -> System.out.println("카테고리가 여행인 책 제목 : " + cate.getBookName()));

        System.out.println("--------------------------------------------------------------------------------------------------------------");

        // 가격이 16200원 이하인 책 제목
        bookList.stream().filter(price -> price.getPrice() <= 16200).forEach(pri -> System.out.println("가격이 16200원 이하인 책 제목 : " + pri.getBookName()));

        System.out.println("--------------------------------------------------------------------------------------------------------------");

        // 책 제목에 "경제"라는 용어가 들어간 책 제목
        bookList.stream().filter(bookname -> bookname.getBookName().contains("경제")).forEach(bookName -> System.out.println("제목에 경제라는 용어가 들어간 책 제목 : " + bookName.getBookName()));

        System.out.println("--------------------------------------------------------------------------------------------------------------");

        // 가격이 가장 비싼 책 가격 조회
        double maxPrice = bookList.stream().mapToDouble(Book::getPrice).max().getAsDouble();
        System.out.println("가격이 가장 비싼 책은 : " + maxPrice + "원");

        System.out.println("--------------------------------------------------------------------------------------------------------------");

        // 카테고리가 IT인 책들의 가격 합
        double sum = bookList.stream().filter(cate -> cate.getCategory().equals("IT")).mapToDouble(Book::getPrice).sum();
        System.out.println("카테고리가 IT인 책들의 가격의 합 : " + sum + "원");

        System.out.println("--------------------------------------------------------------------------------------------------------------");

        // IT 책 할인 이벤트
        List<Book> discountedBookList = bookList.stream().filter(category -> category.getCategory().equals("IT")).map(mappedBoot -> {
            mappedBoot.setPrice(mappedBoot.getPrice() * 0.6);
            return mappedBoot;
        }).toList();
